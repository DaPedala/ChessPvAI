...
<svelte:head>
	<title>{$i18n.auth_register_title} — ChessPvAI</title>
</svelte:head>

<div class="auth-wrap">
	<div class="auth-card">
		<div class="auth-logo">♔</div>
		<h1 class="auth-heading">{$i18n.auth_register_title}</h1>
		<p class="auth-sub">{$i18n.auth_register_sub}</p>
		...
		<form method="POST" use:enhance>
			...
			<div class="field">
				...
				<input id="display_name" name="display_name" type="text" required minlength="2" maxlength="30" autocomplete="username" placeholder="YourName" />
			</div>
			...
			<div class="field">
				<label for="confirm_password">{$i18n.auth_confirm_password}</label>
				<input id="confirm_password" name="confirm_password" type="password" required minlength="8" autocomplete="new-password" placeholder="••••••••" />
			</div>

			<button type="submit" class="btn-submit">{$i18n.auth_register_btn}</button>
		</form>

		<p class="auth-switch">
			{$i18n.auth_have_account}
			<a href="/auth/login">{$i18n.auth_login_link}</a>
		</p>
	</div>
</div>
